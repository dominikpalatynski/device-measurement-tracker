...
describe('DevicesPage', () => {
  ...
  it('renders the devices page correctly', async () => {
    ...
    expect(screen.getByText('Manage and monitor your measurement devices')).toBeInTheDocument();
  ...
  it('deactivates a device successfully after confirmation', async () => {
    ...
    await waitFor(() => {
      expect(mockConfirm).toHaveBeenCalledWith('Are you sure you want to deactivate this device?');
  ...
  it('handles device activation error', async () => {
    ...
    mockDeviceApi.activateDevice.mockRejectedValueOnce(new Error(errorMessage));
  ...
  it('handles device deactivation error', async () => {
    ...
    mockDeviceApi.deactivateDevice.mockRejectedValueOnce(new Error(errorMessage));
  ...
  it('displays correct status colors and text', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText('Pending Registration').length).toBeGreaterThan(0);
  ...
  it('displays correct device type names and icons', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('PMSM Mechanical Vibration')).toBeInTheDocument();
  ...
  it('shows all filters and applies them correctly', async () => {
    ...
    const activeButtons = screen.getAllByText(/Active/);
    const pendingFilterButtons = screen.getAllByText('Pending Registration');
