...
describe("ExperimentDetailPage", () => {
	...
	it("should render the experiment detail page with device and experiment IDs", () => {
		...
		expect(
			screen.getByTestId("experiment-detail-page")
		).toBeInTheDocument();
		expect(
			screen.getByRole("heading", { name: /experiment details/i })
		).toBeInTheDocument();
		expect(screen.getByText(/device id: device-123/i)).toBeInTheDocument();
		expect(
			screen.getByText(/experiment id: experiment-456/i)
		).toBeInTheDocument();
		expect(
			screen.getByText(/experiment management functionality coming soon/i)
		).toBeInTheDocument();
	});
	...
	it("should display different device and experiment IDs when provided", () => {
		...
		expect(
			screen.getByText(/experiment id: test-experiment/i)
		).toBeInTheDocument();
	});
});
